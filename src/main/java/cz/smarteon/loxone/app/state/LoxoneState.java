...
public class LoxoneState implements LoxoneAppListener, LoxoneEventListener {
    ...
    public LoxoneState(LoxoneProfile loxoneProfile) {
    ...
    private void initializeState() {
        controlStates = loxoneApp.getControls().entrySet().stream()
                ...
                .collect(Collectors.toMap(
                        ...
                        e -> {
                            try {
                                return (ControlState<?>) SUPPORTED_CONTROLS_STATE_MAP.get(e.getValue().getClass()).getConstructors()[0].newInstance(loxone, e.getValue());
